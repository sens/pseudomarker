function [lod,logbf] = scan( y, x, z, fake, qtl, model, obsdx )
% SCAN General function for scanning.
% 
% [LOD,LOGBF] = SCAN(Y,X,Z,FAKE)    
% [LOD,LOGBF] = SCAN(Y,X,Z,FAKE,QTL)  
% [LOD,LOGBF] = SCAN(Y,X,Z,FAKE,QTL,MODEL)  
% [LOD,LOGBF] = SCAN(Y,X,Z,FAKE,QTL,MODEL,OBSDX)
%
% LOD = multidimensional array of LOD scores on the log base 10 scale
% LOGBF = log10 of Bayes factor calculated by averaging the LOD scores
%  
...
%	$Revision: 0.834 $ $Date: 2002/01/04 22:54:44 $	
...
while( iposs<nposs )
  ...
  for( ipages = 1:npages )
    ...
    [ rss1, df1 ] = rss( y, [ modelmat x covintmodelmat ] );
    ndx = mysub2ind( [ ipages qtlindex ], [ npages dimlod ] );
    lod( ndx ) = - (n/2) * log( rss1/rawss ); 
    %qtlindex
  end
