function reportpair( lod1, lod2, cv )
% REPORTPAIR Report pairs of loci deemed interesting
%
% REPORTPAIR(LOD1,LOD2,CV)  
...
% See also: FLAG, FLAG2, PERMUTEST2.  
  ...
  penaltyint = 0.5*chi2inv( cv(2), dfint ); % cutoff for interaction
  penaltyadd = 0.5*chi2inv( cv(3), dfadd ); % cutoff for coat tail

  % print the initial message
  fprintf( '\n\tThe following loci were flagged:\n\n' );
  fprintf( 'Chrom\tcM1\tcM2\tLODfull\tLODint\tpval\tLODQ1\tpval\tLODQ2\tpval\n' );
	...
	if( (pint<cv(2)) | ( max(pq1,pq2)<cv(3) ) )
	  lab = strcat( int2str(lod1(i).chrid),':',...
			...
			num2str(100*lod1(j).mpos(s2)), '\t', ...
 			num2str(mmmint/log(10),'%6.2f'), '\t', ...
			num2str(lodint/log(10),'%6.2f'), '\t', ...
			num2str(pint,'%0.4f'), '\t', ...
			num2str(lodq1/log(10),'%6.2f'), '\t', ...
			num2str(pq1,'%0.4f'), '\t', ...
			num2str(lodq2/log(10),'%6.2f'), '\t', ...
			num2str(pq2,'%0.4f'), '\n' );
	  fprintf( lab );
	end
      end
